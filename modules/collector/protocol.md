...
| **ACTIVE\_FLAPS**                 | None      | JSON string of active flaps              | Output of `monitor.GetActiveFlapsSummary()`. Empty string on JSON error.    |
| **AVERAGE\_ROUTE\_CHANGES\_90**   | None      | Floating point number (2 decimal places) | Output of `monitor.GetAverageRouteChanges90()`.                             |
| **INSTANCE**                      | None      | String                                   | The instance name supplied during initialization (`collectorInstanceName`). |
| **VERSION**                       | None      | String                                   | The monitor's program version.                                              |
...
3.  **Monitor Errors:** The monitor responds with `ERROR: ...` for malformed or unknown commands.